...
void handleClient(int clientSocket) {
    ...
    if (nbytes <= 0) {
        ...
        close(clientSocket);
        removeFdFromReactor(nullptr, clientSocket);
    } else {
...
int main() {
    ...
    addFdToReactor(reactor, serverSocket, [&](int fd) {
        clientSocket = accept(fd, (struct sockaddr*)&clientAddr, &addrLen);
        if (clientSocket == -1) {
            cerr << "Error on accept" << endl;
        } else {
            cout << "New connection on socket " << clientSocket << endl;
            addFdToReactor(reactor, clientSocket, handleClient);
        }
    });
